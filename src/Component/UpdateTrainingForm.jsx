...
import { fetchTrainingById, fetchUsers, updateTraining } from '../api';
...
export default function UpdateTrainingForm() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([t, allUsers]) => {
        ...
        setUsers(allUsers);
        setForm({
          trainingModuleName:      t.name ?? '',
          plantDepartment:         t.department ?? '',
          requester_id:            (t.requesters ?? []).map(u => String(u.id ?? u)),
          requester_supervisor_id: (t.requesterSupervisors ?? []).map(u => String(u.id ?? u)),
          typeOfTraining:          t.type_of_training ?? '',
          requirement:             t.requirement ?? '',
          trainingObjectives:      t.training_objectives ?? t.objectives ?? '',
          targetAudience:          t.target_audience ?? '',
          requestedKPIs:           t.requested_kpis ?? t.kpis ?? '',
          desiredPublicationDate:  t.publication_date ? t.publication_date.split('T')[0] : '',
          information:             t.information ?? '',
        });
        setExistingMedia(t.media ?? []);
        setExistingQuizzes(t.quizzes ?? []);
      })
      .catch(err => {
        if (!training) setFetchError(err.message);
        else setLoadError('Impossible de charger les utilisateurs depuis le serveur.');
      })
  ...
  const validate = () => {
    ...
    if (!form.requester_id || form.requester_id.length === 0)
      errs.requester_id = 'This field is required.';
    if (!form.requester_supervisor_id || form.requester_supervisor_id.length === 0)
      errs.requester_supervisor_id = 'This field is required.';
    return errs;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      form.requester_id.forEach(uid             => data.append('requester_id',            uid));
      form.requester_supervisor_id.forEach(uid  => data.append('requester_supervisor_id', uid));
    ...
    return (
      <div className="page-bg">
        ...
        <div className="panel-right" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p style={{ color: '#64748b', fontSize: 15 }}>Loading training data…</p>
        </div>
      </div>
    );
  }
  ...
  if (fetchError) {
    return (
      <div className="page-bg">
        <LeftCarousel />
        <div className="panel-right" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <p style={{ color: '#dc2626', fontSize: 15 }}>Error: {fetchError}</p>
        </div>
      </div>
    );
  }
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className={`fg${errors.requester_supervisor_id ? ' fg-error' : ''}`}>
                ...
                <MultiUserSelect
                  ...
                  onChange={handleChange}
                  placeholder={users.length === 0 ? 'Loading…' : 'Select Requester Supervisor(s)'}
                  disabled={users.length === 0}
                  options={users.map(u => ({ value: u.id, label: `${u.first_name} ${u.last_name}` }))}
                />
              ...
              <FileUploadArea
                ...
                onRemoveServerFile={handleRemoveExistingMedia}
              />
              ...
              {/* Submit */}
              <div className="submit-wrap" style={{ gap: '16px' }}>
                <button
                  type="button"
                  className="upd-cancel-btn"
